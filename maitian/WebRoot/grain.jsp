...
<head>
    ...
    <link href="css/bootstrap/bootstrap-dialog.min.css" rel="stylesheet">
...
<body>
    ...
    <script src="${path}/js/bootstrap/bootstrap-dialog.min.js"></script>
    <script src="${path}/js/global.js"></script>
    <script src="${path}/js/grain.js?v=${rand}"></script>
    
<script type="text/ng-template" id="grainDetail" >
<form id="{gid}"  method="post"  enctype="multipart/form-data">
<input type="hidden" name="key" value="value">
<label>类别:</label><span>{mcateId}</span>
<label>场所名称:</label><span>{siteName}</span>
<label>用户:</label><span>{userId}</span>
<label>推荐级别:</label><span>{recommend}</span>
<label>用户评价:</label><span>{text}</span>
<label>是否公开:</label> <span>{isPublic}</span>
<input class="btn btn-default" type="button" value="新增照片" onclick="Grain.addImageTag('{gid}')">
</form>
</script>
</body>
